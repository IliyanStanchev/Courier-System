...
public class ShipmentDaoImpl extends entityManager implements BaseDao<Shipment> {
	...
	public List<Object[]> getListShipmentsByFirm(int bulstat) {
       ...
       return  getEntityManager().createQuery("SELECT e.id, e.dateCreated,u.phoneNumber,e.status FROM Shipment e, Firm f, User u WHERE f.id=: bulst AND f.id = e.firm AND e.receiver=u.id")
	...
	public Shipment getShipmentByNumAndPhone(int id, String phone) {
		...
		Shipment ship;
		try {
			ship= (Shipment) getEntityManager().createQuery("Select s FROM Shipment s, User u WHERE s.id=: id AND s.receiver = u.id AND u.phoneNumber=: phone")
					.setParameter("id", id)
	...
	public List<Object[]> getListShipmentsByCourier(int courier_id, Status.status status) {
        return getEntityManager().createQuery("SELECT e.id, e.dateCreated,u.phoneNumber,e.status FROM Shipment e, User u, Courier c WHERE e.courier = c.id AND c.id=: courier_id AND e.receiver = u.id AND e.status=: status")
        		.setParameter("courier_id", courier_id)
	...
	public List<Object[]> getAllShipments() {
		  return getEntityManager().createQuery("SELECT e.id, e.dateCreated,u.phoneNumber,e.status FROM Shipment e, User u WHERE e.receiver = u.id")
	                .getResultList();  
	}
